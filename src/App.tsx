...
import { Landmark, ChatMessage, LocalStatusResponse, Barangay } from "./types";
                ...
                <div className="flex flex-col items-center gap-10 relative z-10 py-6">
                    ...
                    {tagbilaranBarangays.map((item, idx) => {
                      ...
                      return (
                        <div
                          ...
                          className={`absolute w-[215px] md:w-[295px] h-[350px] md:h-[460px] rounded-3xl bg-[#fdfbf7] p-5 md:p-6 border-[3px] shadow-2xl transition-all duration-500 [transition-timing-function:cubic-bezier(0.25,1,0.28,1)] cursor-pointer group select-none flex flex-col justify-between overflow-hidden ${
                            diff === 0 
                              ? "border-[#ffaa00] ring-8 ring-[#ffaa00]/10 shadow-[0_30px_70px_rgba(255,170,0,0.3)]" 
                              : "border-[#e0dacf] opacity-65 hover:opacity-100 hover:border-[#ff5722]/30"
                          }`}
                        >
                          {/* Card Double Border Framing Line for Collectible Card Feel */}
                          <div className="absolute inset-1.5 border border-[#e0dacf] rounded-[20px] pointer-events-none group-hover:border-[#ff5722]/20 transition-colors" />

                          {/* Card Top Information */}
                          <div className="relative z-10 w-full">
                            {/* Barangay Title Name block */}
                            <div className="mt-2 border-l-2 pl-2.5" style={{ borderColor: theme.primary }}>
                              <span className="text-[7.5px] font-mono text-zinc-400 tracking-[0.25em] font-extrabold uppercase block leading-none mb-1">
                                BARANGAY
                              </span>
                              <h2 className="font-sans font-black text-xl md:text-3xl text-zinc-900 tracking-tight leading-none uppercase">
                          ...
                          <div className="relative flex items-center justify-center my-auto pointer-events-none select-none">
                            <div className="absolute text-[85px] md:text-[120px] font-sans font-black text-zinc-150/40 select-none uppercase tracking-tighter mix-blend-multiply group-hover:scale-110 transition-transform duration-500">
                              {initialLetter}
                            </div>
                            
                            {/* Decorative badge around monogram */}
                            <div 
                              className="w-16 h-16 md:w-20 md:h-20 rounded-full border border-dashed border-zinc-200 flex items-center justify-center opacity-70 group-hover:rotate-45 transition-transform duration-1000"
                              style={{ borderColor: theme.primary }}
                            />
                          </div>

                          {/* Footer Info details */}
                          <div className="relative z-10 w-full mt-auto pt-3 border-t border-dashed border-zinc-200">
                            <span className="text-[7px] font-mono text-zinc-400 uppercase tracking-widest font-extrabold block mb-1">
                              LEGENDARY HERITAGE FOCUS
                            </span>
                            <div className="flex items-center gap-1.5 text-[10px] md:text-[12px] font-sans font-bold text-zinc-800 leading-none">
                              <span>🚀</span>
                              <span className="truncate">{item.heritage}</span>
                            </div>
                          </div>

                          {/* OVERLAY CARD DETAILS SHEET (Revealed on hover of focused card) */}
                          {diff === 0 && (
                            <div className="absolute inset-0 bg-[#fdfbf7] p-5 md:p-6 flex flex-col justify-between transition-all duration-300 z-30 overflow-hidden text-left translate-y-full opacity-0 pointer-events-none group-hover:translate-y-0 group-hover:opacity-100 group-hover:pointer-events-auto">
                              <div className="absolute inset-1.5 border border-[#e0dacf] rounded-[20px] pointer-events-none" />
                              <div className="relative z-10 flex-1 flex flex-col justify-between">
                                <div>
                                  <div className="flex items-center justify-between">
                                    <span className="font-mono text-[8.5px] text-[#ff5722] font-black tracking-widest uppercase">
                                      {item.category} GUILD
                                    </span>
                                    <span className="text-[7.5px] font-mono bg-zinc-100 text-zinc-500 px-1.5 py-0.5 rounded">
                                      {item.name}
                                    </span>
                                  </div>
                                  
                                  <div className="mt-4">
                                    <p className="text-[10.5px] md:text-[13px] text-zinc-700 leading-relaxed font-sans font-semibold">
                                      {item.desc}
                                    </p>
                                  </div>
                                </div>
                  ...
                  <div className="flex items-center justify-center gap-6 relative z-10 mt-1 sm:mt-3">
                    ...
                    <button 
                      ...
                      className="w-12 h-12 rounded-full border-2 border-white/10 hover:border-[#ff5722] bg-white/5 hover:bg-[#ff5722]/15 text-white flex items-center justify-center cursor-pointer transition-all hover:scale-110 active:scale-95 group"
                      title="Previous Barangay Card"
                    >
                      <span className="text-xl font-bold group-hover:text-[#ffaa00] transition-colors">◀</span>
                    </button>

                    <div className="px-5 py-2 rounded-full bg-white/5 border border-white/10 font-mono text-xs text-white/90">
                      Card Selection <span className="text-[#ffaa00] font-bold">{activeBarangayIdx + 1}</span> of <span className="text-white/60">{tagbilaranBarangays.length}</span>
                    </div>
                    ...
                    <button 
                      ...
                      className="w-12 h-12 rounded-full border-2 border-white/10 hover:border-[#ffaa00] bg-white/5 hover:bg-[#ffaa00]/15 text-white flex items-center justify-center cursor-pointer transition-all hover:scale-110 active:scale-95 group"
                      title="Next Barangay Card"
                    >
                      <span className="text-xl font-bold group-hover:text-[#ff5722] transition-colors">▶</span>
                    </button>
